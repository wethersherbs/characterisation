...
export const generate = fixturify => responsify => composeN(fixturify.bind, sequence(unit), map(map(responsify), testify));

// Run the tests generated for this characterisation suite.
// test : Future e a -> (Response -> String) -> [Test] -> Future Error [Diff]
export const test = fixturify => responsify => composeN(fixturify.bind, sequence(unit), map(map(responsify), comparify));
